 ...
 * 
 * minilzo c# port:
 * http://www.codeproject.com/KB/recipes/managedlzo.aspx
 *      Pure C# MiniLZO port
 *      By Astaelan | 23 Dec 2006
 *      
...
namespace HzNS.MdxLib.Compression.impl
{
    public static class MiniLZO
    {
        ...
        {
            uint tmp;
            var destLen = (uint) (src.Length + (src.Length / 16) + 64 + 3);
            dst = new byte[destLen];
            ...
            if (tmp > 0)
            {
                ...
                if (destLen == 0 && tmp <= 238)
                {
                    dst[destLen++] = (byte) (17 + tmp);
                }
                else if (tmp <= 3)
                {
                    dst[destLen - 2] |= (byte) (tmp);
                }
                else if (tmp <= 18)
                {
                    dst[destLen++] = (byte) (tmp - 3);
                }
                else
                {
                    uint tt = tmp - 18;
            ...
            dst[destLen++] = M4_MARKER | 1;
            dst[destLen++] = 0;
            dst[destLen++] = 0;

            if (dst.Length != destLen)
            {
                byte[] final = new byte[destLen];
                Buffer.BlockCopy(dst, 0, final, 0, (int) destLen);
                dst = final;
            }
        }

        public static unsafe int Decompress(byte[] src, int start, int length, byte[] dst)
        {
            fixed (byte* input = src, output = dst)
            {
                byte* pos = null;
                var ipEnd = input + start + length;
                var opEnd = output + dst.Length;
                var ip = input + start;
                var op = output;
                return _decompress(ip, ipEnd, op, opEnd);
            }
        }

        public static unsafe int Decompress(byte[] src, byte[] dst)
        {
            fixed (byte* input = src, output = dst)
            {
                byte* pos = null;
                var ipEnd = input + src.Length;
                var opEnd = output + dst.Length;
                var ip = input;
                var op = output;
                return _decompress(ip, ipEnd, op, opEnd);
        ...
        private static unsafe int _decompress(byte* ip, byte* ip_end, byte* op, byte* op_end)
        {
            uint t = 0;
            byte* pos = null;
            bool match = false;
            ...
            while (!eof_found && ip < ip_end)
            {
                ...
                if ((int) (ip - input) == 3348)
                {
                    Debug.WriteLine("last");
                }

                Debug.WriteLine(string.Format("    ip = {0}, op = {1}", (int) (ip - input), (int) (op - output)));
            }
        ...
        public unsafe static int DecompressOld(byte[] src, byte[] dst)
        {
            uint t = 0;
            fixed (byte* input = src, output = dst)
            {
